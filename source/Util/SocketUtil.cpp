	...
	const vector<TCPSocketPtr>* inExceptSet, vector<TCPSocketPtr>* outExceptSet)
{
	...
	fd_set* readPtr = SocketUtil::FillSetFromVector(read, inReadSet);
	fd_set* writePtr = SocketUtil::FillSetFromVector(write, inWriteSet);
	fd_set* exceptPtr = SocketUtil::FillSetFromVector(except, inExceptSet);
	int toRet = select(0, readPtr, writePtr, exceptPtr, nullptr);

	if (toRet > 0) {
		SocketUtil::FillVectorFromSet(outReadSet, inReadSet, read);
		SocketUtil::FillVectorFromSet(outWriteSet, inWriteSet, write);
		SocketUtil::FillVectorFromSet(outExceptSet, inExceptSet, except);
	}
	return toRet;
}
