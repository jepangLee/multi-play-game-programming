...
---
- 네트워크 계층의 역할 수행을 위해 논리 주소 체계로 각 호스트마다 개별적인 주소를 부여한다.
...
---
  ...
  ![IPv4 패킷 구조](image/picture_2-2.PNG)
...
### 주소 결정 프로토콜(ARP)
---
- 네트워크 계층의 IP를 링크 계층의 MAC주소로 바인드하는 프로토콜이다.

  ![APR 패킷 구조](image/picture_2-3.png)
...
- 직접 라우팅 예제
  ...
  ![3 대의 호스트가 연결한 네트워크](image/picture_2-4.png)

  그림 2-4) 3 대의 호스트로 구성된 네트워크
  ...
  표 2-3) IP 주소를 MAC 주소로 매핑하는 ARP테이블
...
- B1이 B2에게 데이터를 보내는 과정
      ...
      3. B3은 수신자 IP주소가 다르므로 응답하지 않는다. 하지만 B2는 같기에 ARP패킷으로 응답한다.
      4. B1은 해당 패킷을 받고 B2에 대한 MAC주소를 갱신하고 이를 가리키던 IP패킷을 이더넷 프레임에 포함하여 B2의
        MAC주소로 보낸다. 
...
- 간접 라우팅 예제
  ...
  ![알파와 브라보를 연결한 모습](image/picture_2-5.png)
  
  그림 2-5) 네트워크 알파와 네트워크 브라보를 연결하는 모습
...
- A1이 B1에 패킷을 전달하는 과정
  ...
  ![알파와 브라보를 연결한 모습](image/picture_2-6.png)
  
  그림 2-6) 인터넷에 연결된 네트워크 알파와 네트워크 브라보를 연결하는 모습
  ...
  표 2-7) 인터넷 접근이 가능한 A1, B1, R의 라우팅 테이블
